...
pub var ores: std.ArrayList(*const Self) = undefined;
var _id: Features.FeatureId = std.math.maxInt(Features.FeatureId);
...
pub inline fn id() Blocks.FeatureId {
    return _id;
}
...
fn constructor(instance: *Features.Feature, blockId: u16, zon: ZonElement) void {
    ...
    feature.veins = zon.get(f32, "veins", 0);
    feature.size = zon.get(f32, "size", 0);
    feature.maxHeight = zon.get(i32, "height", 0);
    feature.minHeight = zon.get(i32, "minHeight", std.math.minInt(i32));
    feature.density = zon.get(f32, "density", 0.5);
    feature.blockType = @intCast(blockId);

    ores.append(feature) catch unreachable;
}

const OreGenerator = struct {
    ...
    fn considerCoordinates(ore: *const Self, relX: f32, relY: f32, relZ: f32, chunk: *main.chunk.ServerChunk, startSeed: u64) void {
